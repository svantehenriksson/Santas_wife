...
function Scene({ sceneData, currentSentenceIndex, handleNextSentence, goToNextScene }) {
  return (
    <div className="image-container" style={{ backgroundImage: `url(${sceneData.image})` }}>
      {currentSentenceIndex >= 0 ? (
        ...
        <ChoiceButtons choices={sceneData.choices} onSelect={(choice) => goToNextScene(choice.next)} />
      )}
    </div>
  );
}
